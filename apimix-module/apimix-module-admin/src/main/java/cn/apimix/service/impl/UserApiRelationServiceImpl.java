...
import cn.apimix.service.UserApiRelationService;
 ...
 */
@Service
public class UserApiRelationServiceImpl extends ServiceImpl<UserApiRelationMapper, UserApiRelation> implements UserApiRelationService {

    @Override
    public Page<ApiRelationVo> selectUserApiRelationByPage(ApiInfoQueryRequest pageRequest, Long userId) {
        return pageAs(Page.of(pageRequest.getPageNumber(), pageRequest.getPageSize()),
                query().select(
                        ...
                        .where(ApiInfoTableDef.API_INFO.NAME.like(pageRequest.getName(), StringUtil::isNotBlank))
                , ApiRelationVo.class);
    }
