...
import cn.apimix.service.UserTokenService;
...
public class UserTokenServiceImpl extends ServiceImpl<UserTokenMapper, UserToken> implements UserTokenService {
     ...
     */
    @Override
    public Page<UserToken> selectTokenList(TokenQueryRequest pageRequest, Long userId) {
        // 构建分页
        ...
        QueryWrapper queryWrapper = QueryWrapper.create()
                .where(UserTokenTableDef.USER_TOKEN.USER_ID.eq(userId))
                .and(UserTokenTableDef.USER_TOKEN.NAME.like(pageRequest.getName(), StringUtil::isNotBlank));
     ...
     */
    @Override
    public List<UserToken> selectTokenByUserIdAndApiId(Long userId, Long apiId) {
        List<Long> usedTokens = apiTokenMapper.selectListByQueryAs(query()
                        .select(ApiTokenTableDef.API_TOKEN.TOKEN_ID)
                        .where(ApiTokenTableDef.API_TOKEN.USER_ID.eq(userId))
        ...
        return list(query()
                .where(UserTokenTableDef.USER_TOKEN.USER_ID.eq(userId))
                .and(UserTokenTableDef.USER_TOKEN.ID.notIn(usedTokens)
                ));
    }
}
