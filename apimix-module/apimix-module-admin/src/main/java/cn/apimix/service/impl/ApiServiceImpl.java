...
import cn.apimix.service.ApiService;
 ...
 */
@Service
public class ApiServiceImpl extends ServiceImpl<ApiInfoMapper, ApiInfo> implements ApiService {
    ...
    public Boolean insertApi(ApiAddRequest addRequest, Long userId) {
        ...
        // 新增响应示例
        apiExampleService.saveBatch(result);
    ...
    public ApiInfoVo selectApiInfoByApiId(Long apiId) {
        ...
        List<ApiExample> apiExamples = apiExampleService.list(exampleQueryWrapper);
    ...
    @Override
    @Transactional
    public void updateApiInfo(ApiEditRequest editRequest) {

        // API接口信息
        ApiInfo apiInfo = apiMapping.apiEditRequestToApi(editRequest);
        // 更新接口信息
        updateById(apiInfo);

        // 更新分类
        categoryApiService.update(CategoryApi.builder()
                .apiId(editRequest.getId())
                .categoryId(editRequest.getCategory())
                .build(), query().where(CategoryApiTableDef.CATEGORY_API.API_ID.eq(editRequest.getId())));


        // 更新接口参数
        List<ApiParam> apiParams = new ArrayList<>();
        apiParams.addAll(editRequest.getRequest().getQuery());
        apiParams.addAll(editRequest.getRequest().getHeader());
        apiParams.addAll(editRequest.getRequest().getBody());
        apiParams.addAll(editRequest.getResponse().getHeader());
        apiParams.addAll(editRequest.getResponse().getBody());
        paramService.updateBatch(apiParams);

        // 更新响应示例
        if (!editRequest.getResult().isEmpty()) {
            apiExampleService.updateBatch(editRequest.getResult());
        }

        // 更新套餐
        List<Package> packages = new ArrayList<>();
        editRequest.getPackages().forEach(item -> packages.addAll(item.getList()));
        if (!packages.isEmpty()) {
            packageService.updateBatch(packages);
        }
    ...
    public Page<ApiItemVo> selectApiItemByCategory(ApiInfoQueryRequest queryRequest, Long categoryId) {
        ...
        Page<ApiInfo> apiInfoPage = mapper.paginateWithRelations(
                Page.of(queryRequest.getPageNumber(), queryRequest.getPageSize()),
                query().where(ApiInfoTableDef.API_INFO.ID.in(apiIds))
                        .and(ApiInfoTableDef.API_INFO.STATUS.eq(true))
                        .and(ApiInfoTableDef.API_INFO.NAME.like(queryRequest.getName(), StringUtil::isNotBlank))
        ...
        Page<ApiItemVo> apiItemVoPage = new Page<>();
        apiItemVoPage.setPageNumber(apiInfoPage.getPageNumber());
        apiItemVoPage.setPageSize(apiInfoPage.getPageSize());
        apiItemVoPage.setTotalPage(apiInfoPage.getTotalPage());
        apiItemVoPage.setRecords(apiMapping.apiInfosToApiItemVos(apiInfoPage.getRecords()));
        apiItemVoPage.setTotalRow(apiInfoPage.getTotalRow());
        return apiItemVoPage;
    ...
    public Page<ApiInfo> selectDevApiInfoByPage(ApiInfoQueryRequest queryRequest, Long userId) {
        QueryWrapper queryWrapper = QueryWrapper.create()
                ...
                .and(ApiInfoTableDef.API_INFO.NAME.like(queryRequest.getName(), StringUtil::isNotBlank))
                .and(ApiInfoTableDef.API_INFO.STATUS.eq(queryRequest.getStatus()));
