...
import cn.apimix.user.mapper.RoleMapper;
import cn.apimix.user.model.entity.Role;
import cn.apimix.user.model.entity.RoleMenu;
import cn.apimix.user.model.entity.table.RoleTableDef;
...
import cn.apimix.user.service.RoleService;
...
public class RoleServiceImpl extends ServiceImpl<RoleMapper, Role> implements RoleService {
     ...
     * 新增角色
     *
     * @param addRequest 角色实体
     * @return 是否成功
     */
    @Transactional
    @Override
    public Boolean insertRole(SysRoleAddRequest addRequest) {
        ...
        // 拿到菜单ID列表
        List<Integer> menuIds = addRequest.getMenuIds();
        Integer id = role.getId();

        List<RoleMenu> roleMenus = new ArrayList<>();

        menuIds.forEach(menuId -> roleMenus.add(RoleMenu.builder()
                        .roleId(id)
                        .menuId(menuId)
                .build()));
    ...
    public Boolean deleteRoleById(Integer roleId) {
        ...
        // 判断角色是否能被删除
        Assert.isFalse(role.getType(), "内置角色无法删除");
        // 判断角色是否被分配
