...
public class ApiLogServiceImpl extends ServiceImpl<ApiLogMapper, ApiLog> implements IApiLogService {
    ...
    public List<Xy> getCountByUserIdAndYearMonth(Long userId) {
        // 获取当前日期
        ...
        // 生成从当前月往前12个月的列表
        List<Xy> fullYearXys = IntStream.rangeClosed(currentDate.getMonthValue() - 11, currentDate.getMonthValue())
                .mapToObj(i -> {
                    int month = i < 1? i + 12 : i;
                    int year = currentDate.getYear() - (i < 1? 1 : 0);
                    return new Xy(LocalDate.of(year, month, 1).format(formatter), 0L);
                })
                .collect(Collectors.toList());

        Map<String, Long> map = Collections.singletonMap("user_id", userId);
        List<Row> rows = Db.selectListBySql("SELECT  DATE_FORMAT(request_time, '%Y-%m') as x,count(*) as y FROM api_log WHERE user_id = #{user_id} \n" +
        ...
        List<Xy> xys = RowUtil.toEntityList(rows, Xy.class);

        // 将原始查询到的数据填充到完整列表中
        xys.forEach(xy -> fullYearXys.stream()
                .filter(f -> f.getX().equals(xy.getX()))
                .findFirst()
                .ifPresent(f -> f.setY(xy.getY())));
    ...
    public List<Xy> getCountIpByUserId(Long userId) {
        // 获取当前日期
        LocalDate currentDate = LocalDate.now();
        ...
        List<Xy> xys = listAs(query().select(
                QueryMethods.dateFormat(ApiLogTableDef.API_LOG.REQUEST_TIME, "%Y-%m").as(Xy::getX),
                QueryMethods.count(QueryMethods.distinct(ApiLogTableDef.API_LOG.IP)).as(Xy::getY)
        ).where(ApiLogTableDef.API_LOG.USER_ID.eq(userId)).groupBy(QueryMethods.dateFormat(ApiLogTableDef.API_LOG.REQUEST_TIME, "%Y-%m")), Xy.class);
